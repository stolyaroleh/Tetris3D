...
class Tools2D {
    ...
    static void triangulate(Point2D[] P, Triangle[] tr) {  // P contains all n polygon vertices in CCW order.
        ...
        for (int k = 0; k < n - 2; k++) {  // Find a suitable triangle, consisting of two edges
    ...
    static float distance2(Point2D P, Point2D Q) {
        float dx = (float) (P.x - Q.x), dy = (float) (P.y - Q.y);
        return dx * dx + dy * dy;
    }
}
